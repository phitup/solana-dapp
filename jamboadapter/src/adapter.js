"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.JamboWalletAdapter = exports.JamboWalletName = void 0;
var wallet_adapter_base_1 = require("@solana/wallet-adapter-base");
var web3_js_1 = require("@solana/web3.js");
exports.JamboWalletName = "Jambo";
var JamboWalletAdapter = /** @class */ (function (_super) {
    __extends(JamboWalletAdapter, _super);
    function JamboWalletAdapter(config) {
        if (config === void 0) { config = {}; }
        var _this = _super.call(this) || this;
        _this.name = exports.JamboWalletName;
        _this.url = "https://www.jambo.technology/";
        _this.icon = "data:image/svg+xml;base64,";
        _this.supportedTransactionVersions = null;
        _this._readyState = typeof window === "undefined" || typeof document === "undefined"
            ? wallet_adapter_base_1.WalletReadyState.Unsupported
            : wallet_adapter_base_1.WalletReadyState.NotDetected;
        _this._disconnected = function () {
            var wallet = _this._wallet;
            if (wallet) {
                wallet.off("disconnect", _this._disconnected);
                _this._wallet = null;
                _this._publicKey = null;
                _this.emit("error", new wallet_adapter_base_1.WalletDisconnectedError());
                _this.emit("disconnect");
            }
        };
        _this._connecting = false;
        _this._wallet = null;
        _this._publicKey = null;
        if (_this._readyState !== wallet_adapter_base_1.WalletReadyState.Unsupported) {
            wallet_adapter_base_1.scopePollingDetectionStrategy(function () {
                var _a;
                if ((_a = window.jambo) === null || _a === void 0 ? void 0 : _a.isJambo) {
                    _this._readyState = wallet_adapter_base_1.WalletReadyState.Installed;
                    _this.emit("readyStateChange", _this._readyState);
                    return true;
                }
                return false;
            });
        }
        return _this;
    }
    Object.defineProperty(JamboWalletAdapter.prototype, "publicKey", {
        get: function () {
            return this._publicKey;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(JamboWalletAdapter.prototype, "connecting", {
        get: function () {
            return this._connecting;
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(JamboWalletAdapter.prototype, "connected", {
        get: function () {
            var _a;
            return !!((_a = this._wallet) === null || _a === void 0 ? void 0 : _a.isConnected);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(JamboWalletAdapter.prototype, "readyState", {
        get: function () {
            return this._readyState;
        },
        enumerable: false,
        configurable: true
    });
    JamboWalletAdapter.prototype.connect = function () {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, error_1, publicKey, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 5, 6, 7]);
                        if (this.connected || this.connecting)
                            return [2 /*return*/];
                        if (this._readyState !== wallet_adapter_base_1.WalletReadyState.Installed)
                            throw new wallet_adapter_base_1.WalletNotReadyError();
                        this._connecting = true;
                        wallet = window.jambo;
                        if (!!wallet.isConnected) return [3 /*break*/, 4];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, wallet.connect()];
                    case 2:
                        _a.sent();
                        return [3 /*break*/, 4];
                    case 3:
                        error_1 = _a.sent();
                        throw new wallet_adapter_base_1.WalletConnectionError(error_1 === null || error_1 === void 0 ? void 0 : error_1.message, error_1);
                    case 4:
                        if (!wallet.publicKey)
                            throw new wallet_adapter_base_1.WalletAccountError();
                        publicKey = void 0;
                        try {
                            publicKey = new web3_js_1.PublicKey(wallet.publicKey.toBytes());
                        }
                        catch (error) {
                            throw new wallet_adapter_base_1.WalletPublicKeyError(error === null || error === void 0 ? void 0 : error.message, error);
                        }
                        wallet.on("disconnect", this._disconnected);
                        this._wallet = wallet;
                        this._publicKey = publicKey;
                        this.emit("connect", publicKey);
                        return [3 /*break*/, 7];
                    case 5:
                        error_2 = _a.sent();
                        this.emit("error", error_2);
                        throw error_2;
                    case 6:
                        this._connecting = false;
                        return [7 /*endfinally*/];
                    case 7: return [2 /*return*/];
                }
            });
        });
    };
    JamboWalletAdapter.prototype.disconnect = function () {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        wallet = this._wallet;
                        if (!wallet) return [3 /*break*/, 4];
                        wallet.off("disconnect", this._disconnected);
                        this._wallet = null;
                        this._publicKey = null;
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, wallet.disconnect()];
                    case 2:
                        _a.sent();
                        return [3 /*break*/, 4];
                    case 3:
                        error_3 = _a.sent();
                        this.emit("error", new wallet_adapter_base_1.WalletDisconnectionError(error_3 === null || error_3 === void 0 ? void 0 : error_3.message, error_3));
                        return [3 /*break*/, 4];
                    case 4:
                        this.emit("disconnect");
                        return [2 /*return*/];
                }
            });
        });
    };
    JamboWalletAdapter.prototype.signTransaction = function (transaction) {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, error_4, error_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 5, , 6]);
                        wallet = this._wallet;
                        if (!wallet)
                            throw new wallet_adapter_base_1.WalletNotConnectedError();
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, wallet.signTransaction(transaction)];
                    case 2: return [2 /*return*/, ((_a.sent()) || transaction)];
                    case 3:
                        error_4 = _a.sent();
                        throw new wallet_adapter_base_1.WalletSignTransactionError(error_4 === null || error_4 === void 0 ? void 0 : error_4.message, error_4);
                    case 4: return [3 /*break*/, 6];
                    case 5:
                        error_5 = _a.sent();
                        this.emit("error", error_5);
                        throw error_5;
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    JamboWalletAdapter.prototype.signAllTransactions = function (transactions) {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, error_6, error_7;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 5, , 6]);
                        wallet = this._wallet;
                        if (!wallet)
                            throw new wallet_adapter_base_1.WalletNotConnectedError();
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, wallet.signAllTransactions(transactions)];
                    case 2: return [2 /*return*/, ((_a.sent()) ||
                            transactions)];
                    case 3:
                        error_6 = _a.sent();
                        throw new wallet_adapter_base_1.WalletSignTransactionError(error_6 === null || error_6 === void 0 ? void 0 : error_6.message, error_6);
                    case 4: return [3 /*break*/, 6];
                    case 5:
                        error_7 = _a.sent();
                        this.emit("error", error_7);
                        throw error_7;
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    JamboWalletAdapter.prototype.signMessage = function (message) {
        return __awaiter(this, void 0, void 0, function () {
            var wallet, signature, error_8, error_9;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 5, , 6]);
                        wallet = this._wallet;
                        if (!wallet)
                            throw new wallet_adapter_base_1.WalletNotConnectedError();
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, wallet.signMessage(message)];
                    case 2:
                        signature = (_a.sent()).signature;
                        return [2 /*return*/, signature];
                    case 3:
                        error_8 = _a.sent();
                        throw new wallet_adapter_base_1.WalletSignMessageError(error_8 === null || error_8 === void 0 ? void 0 : error_8.message, error_8);
                    case 4: return [3 /*break*/, 6];
                    case 5:
                        error_9 = _a.sent();
                        this.emit("error", error_9);
                        throw error_9;
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    return JamboWalletAdapter;
}(wallet_adapter_base_1.BaseMessageSignerWalletAdapter));
exports.JamboWalletAdapter = JamboWalletAdapter;
